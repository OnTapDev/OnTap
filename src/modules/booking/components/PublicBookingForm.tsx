  ...
  return (
    <div className="bg-warm-sand/5 rounded-lg p-3 select-none border border-warm-sand/10">
      <div className="flex items-center justify-between mb-3">
        <button type="button" onClick={goToPrevMonth} className="p-1 text-warm-white/50 hover:text-warm-white transition-colors">
          <ChevronLeft className="w-4 h-4" />
        </button>
        <span className="text-sm font-bold text-warm-white">{monthNames[month]} {year}</span>
        <button type="button" onClick={goToNextMonth} className="p-1 text-warm-white/50 hover:text-warm-white transition-colors">
          <ChevronRight className="w-4 h-4" />
        </button>
      </div>
      <div className="grid grid-cols-7 gap-0 text-center mb-1">
        {["S", "M", "T", "W", "T", "F", "S"].map(d => (
          <div key={d} className="text-xs text-warm-white/50 font-medium py-1">{d}</div>
        ))}
      </div>
  ...
  if (step === "success") {
    ...
    return (
      <div className="max-w-lg mx-auto text-center py-16 px-6">
        ...
        {hasDeposit && (
          <div className="bg-warm-sand/5 rounded-xl p-6 border border-warm-sand/10 mb-6 text-left">
            <h3 className="text-warm-white font-semibold mb-2">Secure Your Date</h3>
            ...
            <div className="flex items-center justify-between text-sm text-warm-white/60 mb-3">
              <span>Estimated total</span>
              <span className="text-warm-white font-bold">${pricing!.total.toLocaleString()}</span>
            </div>
            ...
            <p className="text-xs text-warm-white/30 mt-3 text-center">Payments are processed securely via Stripe. You can also pay later.</p>
          </div>
        )}
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-2xl mx-auto space-y-8">
      <div className="text-center">
        <h1 className="text-3xl font-bold text-warm-white mb-2">Book {orgName}</h1>
        <p className="text-warm-white/70">Fill in the details below and we&apos;ll get back to you promptly.</p>
      </div>
      ...
      <div className="bg-warm-sand/5 rounded-xl p-6 border border-warm-sand/10 space-y-4">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-warm-white/80 mb-1.5">Name *</label>
            <input
              type="text"
              value={name}
          ...
          <div>
            <label className="block text-sm font-medium text-warm-white/80 mb-1.5">Email *</label>
            <input
              type="email"
              value={email}
          ...
          <div className="sm:col-span-2">
            <label className="block text-sm font-medium text-warm-white/80 mb-1.5">Phone</label>
            <input
              type="tel"
              value={phone}
      ...
      <div className="bg-warm-sand/5 rounded-xl p-6 border border-warm-sand/10 space-y-4">
        ...
        <div>
          <label className="block text-sm font-medium text-warm-white/80 mb-1.5">Event Name *</label>
          <input
            type="text"
            value={eventName}
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-warm-white/80 mb-1.5">Event Type *</label>
            <select
              value={eventType}
          ...
          <div>
            <label className="block text-sm font-medium text-warm-white/80 mb-1.5">Estimated Guests</label>
            <input
              type="number"
              value={guestCount}
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <div>
            <label className="block text-sm font-medium text-warm-white/80 mb-1.5">Start Time</label>
            <input
              type="time"
              value={startTime}
          ...
          <div>
            <label className="block text-sm font-medium text-warm-white/80 mb-1.5">End Time</label>
            <input
              type="time"
              value={endTime}
        ...
        <div>
          <label className="block text-sm font-medium text-warm-white/80 mb-1.5">Venue Name</label>
          <input
            type="text"
            value={venueName}
        ...
        <div>
          <label className="block text-sm font-medium text-warm-white/80 mb-1.5">Notes</label>
          <textarea
            value={notes}
        ...
        <div className="bg-warm-sand/5 rounded-xl p-6 border border-warm-sand/10 space-y-4">
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap gap-2 mt-2">
                        <span className="text-xs text-warm-white/50 bg-warm-sand/5 px-2 py-0.5 rounded">
                          {pkg.includes_bartenders} bartender{pkg.includes_bartenders !== 1 ? "s" : ""}
                        </span>
                        {pkg.includes_glassware && (
                          <span className="text-xs text-warm-white/50 bg-warm-sand/5 px-2 py-0.5 rounded">Glassware</span>
                        )}
                      </div>
                    </div>
          ...
          {selectedPackage && pricing && (
            <div className="bg-warm-sand/5 rounded-lg p-4 border border-warm-sand/10">
              <div className="flex justify-between text-sm text-warm-white/60 mb-1">
                <span>Estimated subtotal</span>
                <span className="text-warm-white">${pricing.subtotal.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-sm text-warm-white/60 mb-1">
                <span>Estimated tax</span>
                <span className="text-warm-white">${pricing.tax.toLocaleString()}</span>
              </div>
      ...
      <p className="text-center text-xs text-warm-white/30 pb-8">
